export default class Validate {
  constructor(forms, button) {
    ...
    this.errorMessage = {
      ...
      errorEmail: { rus: 'Неправильный формат email' },
      errorSignUp: { rus: 'Такой пользователь уже есть' },
  ...
  linkError(e) {
    e.target.nextElementSibling.textContent = this.errorMessage.errorEmail.rus;
    e.target.nextElementSibling.classList.add('popup__error_invalid');
  }
  ...
  signUpError(e) {
    e.target.nextElementSibling.textContent = this.errorMessage.errorSignUp.rus;
    e.target.nextElementSibling.classList.add('popup__error_invalid');
  }
